...
<form class="bs-example form-horizontal validate" data-validate="parsley" action="<?php echo base_url() ?>costumer/useredit?username=<?php if (isset($username2)) echo $username2; else echo $data['username']; ?>"	method="post" enctype="multipart/form-data">
				...
				<div class="form-group">
					...
					<?php
						foreach($cabang->result_array() as $row){
							echo "<option value='".$row['cabang']."' ".($data['cabang']==$row['cabang']?'selected':'').">".$row['cabang']."</option>";
						}
					?>
					</select>
					</div>
				...
				<div class="form-group">
					...
					<div class="col-lg-8">
						<select class="form-control" name="rule">
							...
							<option value="Audit" <?php echo ($data['rule']=='Audit'?'selected':'') ?> >Audit</option>
							<option value="Manager" <?php echo ($data['rule']=='Manager'?'selected':'') ?> >Manager</option>
							<option value="Helper" <?php echo ($data['rule']=='Helper'?'selected':'') ?> >Helper</option>
						</select>
					</div>
				</div>
				<div class="form-group">
